document.addEventListener('DOMContentLoaded', () => {
    ...
    function displayBooks(books) {
        ...
        books.forEach(book => {
            ...
            bookDiv.innerHTML = `
                ...
                <button class="addToCurrentlyReading" data-id="${book.id}" data-title="${title}" data-authors="${authors}" data-image="${imageUrl}">Add to Currently Reading</button>
            `;
            bookList.appendChild(bookDiv);

            // Add book to To-Read list
            const addToToReadButton = bookDiv.querySelector('.addToReadingList');
            addToToReadButton.addEventListener('click', (e) => {
                const bookDetails = {
            ...
            const addToCurrentlyReadingButton = bookDiv.querySelector('.addToCurrentlyReading');
            addToCurrentlyReadingButton.addEventListener('click', (e) => {
                const bookDetails = {
                    id: e.target.getAttribute('data-id'),
                    title: e.target.getAttribute('data-title'),
                    authors: e.target.getAttribute('data-authors'),
                ...
                addToReadingList(bookDetails, 'currentlyReading');
            });
        });
    }
    ...
    function appendBookToList(book, listName) {
        ...
        li.innerHTML = `
            ...
            ${listName === 'currentlyReading' ? 
                `<button class="markAsCompleted" data-id="${book.id}" data-title="${book.title}" data-authors="${book.authors}" data-image="${book.imageUrl}">Mark as Completed</button>` 
                : ''}
        `;
        ...
        if (listName === 'currentlyReading') {
            const markAsCompletedButton = li.querySelector('.markAsCompleted');
            markAsCompletedButton.addEventListener('click', (e) => {
                const bookDetails = {
                    id: e.target.getAttribute('data-id'),
                    title: e.target.getAttribute('data-title'),
                    authors: e.target.getAttribute('data-authors'),
                ...
                markBookAsCompleted(bookDetails);
            });
        }
    }
    ...
    function removeFromReadingList(bookId, listName) {
        let readingList = JSON.parse(localStorage.getItem(listName)) || [];
        readingList = readingList.filter(book => book.id !== bookId); // Remove the book by id
        localStorage.setItem(listName, JSON.stringify(readingList));
        loadReadingLists(); // Reload the lists in the UI
    }

    // Mark a book as completed (move it from Currently Reading to Completed)
    function markBookAsCompleted(bookDetails) {
        // Remove the book from the currently reading list
        let currentlyReading = JSON.parse(localStorage.getItem('currentlyReading')) || [];
        currentlyReading = currentlyReading.filter(book => book.id !== bookDetails.id);

        // Add the book to the completed list
        let completed = JSON.parse(localStorage.getItem('completed')) || [];
        completed.push(bookDetails);

        // Update the localStorage
        localStorage.setItem('currentlyReading', JSON.stringify(currentlyReading));
        localStorage.setItem('completed', JSON.stringify(completed));

        // Reload the lists in the UI
        loadReadingLists();
    }
